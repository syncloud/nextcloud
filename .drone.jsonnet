...
local build(arch, platform_image) = {
    ...
    steps: [
        ...
        },
        {
            ...
            commands: [
              "apt-get update && apt-get install -y sshpass openssh-client netcat rustc file libxml2-dev",
        ...
        }] + ( if arch == "arm" then [] else [
        {
            ...
            image: "python:3.9-slim-buster",
            commands: [
              "apt-get update && apt-get install -y sshpass openssh-client libxml2-dev",
              "pip install -r dev_requirements.txt",
              "DOMAIN=$(cat domain)",
              "cd integration",
