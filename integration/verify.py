...
def test_occ_mail(device, device_user, artifact_dir):
    print(check_output('set -x; nc syncloud.test:25 < /drone/src/integration/mail.test > {0}/mail.test.log 2>&1'.format(artifact_dir), shell=True))
